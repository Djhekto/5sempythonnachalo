...
for (el, value) in codes_len1.items():
    for (i, j) in itertools.permutations(cods, 2):	
        if el[0][0] == i:
            continue
        try:
            codes_len2[((j, i),) + el] = value.decode(i).encode(j)
            if startEnd.encode(i).decode(el[0][0]).encode(el[0][1]).decode(s2) == s1:	
                for seq in inst1:
                    print(seq.encode(i).decode(el[ii][ii]).encode(el[ii][1]).decode(s2))
                sys.exit()
